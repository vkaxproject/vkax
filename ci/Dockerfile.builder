...
# jgc_hash
RUN git clone https://github.com/jgcproject/jgc_hash
RUN cd jgc_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} jgc
RUN useradd -u ${USER_ID} -g jgc -s /bin/bash -m -d /jgc jgc
...
RUN mkdir /jgc-src && \
  ...
  chown $USER_ID:$GROUP_ID /jgc-src && \
