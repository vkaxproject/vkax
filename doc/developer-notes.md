...
- [Development guidelines](#development-guidelines)
    - [General Jagoan Core](#general-dash-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Jagoan Core is a multi-threaded application, and deadlocks or other
...
$ valgrind -v --leak-check=full src/jgcd -printtoconsole
```
...
Jagoan Core can be compiled with various "sanitizers" enabled, which add
...
- ThreadSocketHandler : Sends/Receives data from peers on port 11110.
...
However, in open source software such as Jagoan Core, where everyone uses
...
pay attention to for reviewers of Jagoan Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Jagoan Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Jagoan Core so that they can be integrated
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Jagoan Core, the wallet was always
