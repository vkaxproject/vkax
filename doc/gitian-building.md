...
*Setup instructions for a Gitian build of Vkax Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Vkax
...
- [Building Vkax Core](#building-dash-core)
...
Clone the git repositories for Vkax Core and Gitian.
...
in the Vkax Core repository under 'Fetch and create inputs' to install sources which require
...
To build Vkax Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#setup-and-perform-gitian-builds) in the Vkax Core repository.
