...
void SetupChainParamsBaseOptions()
{
    ...
    gArgs.AddArg("-llmqblocklocks=<quorum name>", "Override the default LLMQ type used for BlockLocks. Allows using BlockLocks with smaller LLMQs. (default: llmq_50_60, devnet-only)", ArgsManager::ALLOW_ANY, OptionsCategory::CHAINPARAMS);
...
std::unique_ptr<CBaseChainParams> CreateBaseChainParams(const std::string& chain)
{
    if (chain == CBaseChainParams::MAIN)
        return MakeUnique<CBaseChainParams>("", 11111);
    else if (chain == CBaseChainParams::TESTNET)
        return MakeUnique<CBaseChainParams>("testnet3", 22222);
    else if (chain == CBaseChainParams::DEVNET)
        return MakeUnique<CBaseChainParams>(gArgs.GetDevNetName(), 33333);
    else if (chain == CBaseChainParams::REGTEST)
        return MakeUnique<CBaseChainParams>("regtest", 44444);
    else
