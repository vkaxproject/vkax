...
static bool AppInit(int argc, char* argv[])
{
    ...
    // If Qt is used, parameters/vkax.conf are parsed in qt/bitcoin.cpp's main()
    ...
    if (HelpRequested(args) || args.IsArgSet("-version")) {
        ...
        if (args.IsArgSet("-version")) {
            ...
            strUsage += "\nUsage:  vkaxd [options]                     Start " PACKAGE_NAME " Daemon\n";
            strUsage += "\n" + args.GetHelpMessage();
        }
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see vkaxd -h for a list of options.\n", argv[i])));
            }
        }
