                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
        ...
        for (const auto& tx : block.vtx) {
            ...
            while (llmq::CInstantSendLockPtr conflictLock = m_isman->GetConflictingLock(*tx)) {
                if (m_clhandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                    LogPrint(BCLog::ALL, "ConnectBlock(VKAX): chain-locked transaction %s overrides islock %s\n",
                    ...
                    return state.Invalid(ValidationInvalidReason::TX_CONFLICT_LOCK, error("ConnectBlock(VKAX): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), conflictLock->txid.ToString()), REJECT_INVALID, "conflict-tx-lock");
                }
            }
        }
    }
    ...
    if (!IsBlockValueValid(*sporkManager, *governance, block, pindex->nHeight, blockReward, strError)) {
        ...
        return state.Invalid(ValidationInvalidReason::NONE, error("ConnectBlock(VKAX): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }
    ...
    if (!IsBlockPayeeValid(*sporkManager, *governance, *block.vtx[0], pindex->nHeight, blockReward)) {
        // NOTE: Do not punish, the node might be missing governance data
        return state.Invalid(ValidationInvalidReason::NONE, error("ConnectBlock(VKAX): couldn't find masternode or superblock payments"), REJECT_INVALID, "bad-cb-payee");
    }
