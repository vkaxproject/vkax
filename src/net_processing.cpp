...
#if defined(NDEBUG)
# error "Jagoan Core cannot be compiled without assertions."
#endif
 ...
 *  We have 4 times smaller block times in Jagoan, so we need to push 4 times more invs per 1MB. */
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /*
        Jagoan Related Inventory Messages
...
bool PeerLogicValidation::SendMessages(CNode* pto)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        // JGC this code also handles non-TXs (Jagoan specific messages)
